...
const checks: Array<[string, boolean]> = [
  ...
  ["DEF-03: locked items remain focusable and expose a reason", desk.includes("aria-disabled={item.disabled || undefined}") && desk.includes("aria-describedby={item.disabled ? tooltipId : undefined}") && desk.includes('role="tooltip" className="lpos-rail-tooltip"') && css.includes(':focus-visible .lpos-rail-tooltip { display: block;')],
  ...
  ["NEW-01: active Living Procurement OS stage uses Netify orange", css.includes('.lpos-header.nf-2030-journey .nf-2030-lifecycle button[data-current="true"]::before { background: var(--lpos-orange);')],
];
