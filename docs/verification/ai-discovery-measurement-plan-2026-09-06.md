...
Google Enhanced Measurement was observed extracting q= into search_term despite a sanitized page_location. The shipped guard therefore restricts GA4 to clean public routes and uses the documented ga-disable property before private/query-bearing navigation. Once opted out, that document remains opted out so queued private events cannot flush. Private flows retain sanitized consent-dependent Vercel events and the server outcome report. Tests exercise the real Google loader and intercept outbound collection, including history changes; no private query text was present after the fix. This does not prove GA4/Vercel dashboard ingestion. Official references: https://developers.google.com/tag-platform/security/guides/privacy, https://developers.google.com/analytics/devguides/collection/ga4/views and https://vercel.com/docs/analytics/package.
