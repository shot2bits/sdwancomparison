 ...
 * (docs/netify-rfp-flow-spec-2026-07-14.md). Six micro-steps, one decision
 * per screen, with a live supplier-match panel so the value proposition is
 * shown as a dataset fact while the buyer answers. No server draft exists
 ...
 *
 * Conversion rework (21 July 2026, Robert's build order; pillar strategy §5,
 * serves Record + Marketplace conversion): the flow now opens on the scope
 * cards, one click, no typing, and that first click lights up the live
 * supplier match. The project title, previously the blank-page first ask,
 * is drafted automatically from the answers and shown editable at the
 * agreement step, where suppliers-see-this-first makes it meaningful.
 * Evidence: 55 start-intent clicks against 17 describe_started in 30 days;
 * a title of at least eight characters was the first hurdle.
 */
...
export default function DescribeWizard() {
  ...
  // Public Opportunities Board listing (sector and size band only, never the
  // company name -- buyer_visibility is anonymous by construction in the
  // publish core) is no longer an optional, untickable choice here.
  // Market-unlock correction round 2 (16 Aug 2026), Robert's non-negotiable
  // product rule: vendor identities, matching, invitations, supplier-room
  // access and everything else supplier-facing unlock ONLY after this RFP
  // has been successfully published as a PUBLIC opportunity on the board --
  // an unlisted/private listing never satisfies that rule, so unticking this
  // used to silently promise "your matched vendors only" while, under the
  // corrected saga, it now means nothing ever unlocks at all. There is no
  // real "matched vendors only" path any more, so this is no longer a
  // consent toggle; it is always true, and the copy below says so plainly
  // instead of offering a choice that would leave the buyer's RFP stuck
  // market-locked with no explanation.
  ...
  // True once the buyer edits the title by hand (or arrives with ?title=);
  // until then the title tracks the answers via the auto-draft below.
  const titleEdited = useRef(false);

  // Pre-answer from entry links (?scope=sdwan&sector=healthcare), editable.
  // A valid ?scope= answers the opening screen, so those arrivals land on
  // estate sizing with their scope already selected. The homepage hero form
  // carries ?title=, which pins the title (still editable at the agreement).
  useEffect(() => {
  ...
  // faces a blank page. Region labels are safe here; site bands use their
  // keys so no en dash reaches a supplier-facing title.
  function autoTitle(): string {
    const scopeLabel =
      scope === "sdwan" ? "SD-WAN" :
      scope === "sse" ? "SSE" :
      scope === "sase" ? "SASE" :
      scope === "managed" ? "Managed SASE service" :
      "SASE & SD-WAN project";
    const prefix = model === "managed" && scope !== "managed" ? "Fully managed " : model === "co_managed" ? "Co-managed " : "";
    const parts = [`${prefix}${scopeLabel}`];
    if (sites) parts.push(`${sites} sites`);
    if (regions.length) {
      const names = regions.map((k) => REGIONS.find((r) => r.key === k)?.label ?? k);
      parts.push(names.slice(0, 2).join(" and ") + (names.length > 2 ? " and more" : ""));
    }
    const composed = parts.join(", ").slice(0, 120);
    return composed.length >= 8 ? composed : `${scopeLabel} requirement`;
  }
  ...
  async function create(submit: boolean) {
    ...
    const finalTitle = title.trim().length >= 8 ? title.trim() : autoTitle();
    ...
    try {
      ...
      if (!id) {
        ...
        const pending_submit = submit ? { shortlist_size: 5, list_on_board: true, marketing_opt_in: optIn } : undefined;
      ...
      try {
        ...
        localStorage.setItem(`rfp_publish_opts_${id}`, JSON.stringify({ shortlist_size: 5, list_on_board: true, marketing_opt_in: optIn }));
  ...
  return (
    <div>
      ...
      <div className="max-w-xl">
        ...
        {step === 0 && (
          <div>
            {heading("What are you buying?", "One click starts your brief. Pick the closest fit; you can refine it later, and your project title is drafted for you.")}
            <div className="space-y-2">
              {SCOPES.map((s) => (
                <button key={s.key} onClick={() => { markStarted(); setScope(s.key); if (s.key === "managed") setModel("managed"); advance(1, "scope"); }} className={`${card} ${scope === s.key ? active : idle}`}>
        ...
        {step === 1 && (
          <div>
            ...
            <div className="mt-5 flex items-center gap-3">
              <button onClick={() => advance(2, "estate")} disabled={!sites || regions.length === 0} className={nextBtn}>Continue</button>
              <button onClick={() => setStep(0)} className={backBtn}>Back</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <div className="mt-5 flex items-center gap-3">
              <button onClick={() => advance(3, "context")} className={nextBtn}>Continue</button>
        ...
        {step === 3 && (
          <div>
            ...
            <div className="mt-5 flex items-center gap-3">
              <button onClick={() => advance(4, "timeline")} className={nextBtn}>Continue</button>
        ...
        {step === 4 && (
          <div>
            ...
            <div className="mt-5 flex items-center gap-3">
              <button onClick={() => setStep(5)} className={nextBtn}>Continue</button>
        ...
        {step === 5 && (
          <div>
            {heading("Generate and publish your opportunity", "Built for UK and North American businesses with national or global network requirements.")}
            {match && match.count > 0 && (
              <p className="mb-3 text-sm text-[var(--ink-700)]">
                Going to: <strong>{match.names.slice(0, Math.min(5, match.count)).join(", ")}</strong>.
                {match.count > 5 ? ` ${match.count - 5} more match; you can add them after publishing.` : " You can add more after publishing."}
              </p>
            )}
            <div className="mb-3">
            ...
            {authed === true ? (
              ...
              <div className="mb-3">
                ...
                <input
                  ...
                  className="w-full max-w-md border border-[var(--ink-300,#ccc)] rounded-sm p-3 text-base"
                />
                <p className="mt-1 text-xs text-[var(--ink-500)]">Business addresses only. We email a confirm link and a 6-digit code; either completes publication (the code works on this screen, no link-hunting).</p>
              </div>
            )}
            ...
            <div className="mb-4 rounded-sm border border-emerald-200 bg-emerald-50/60 p-4">
              <p className="text-sm font-semibold mb-2">What you get the moment you publish</p>
              <ul className="space-y-1.5 text-sm text-[var(--ink-700)]">
                ...
                <li className="flex gap-2"><span aria-hidden="true" className="text-emerald-600 font-bold">✓</span> Your RFP is published as a public opportunity and goes to your {Math.min(5, match?.count ?? 5) || 5} matched vendors, each with a private response link; structured responses come back side by side</li>
        ...
        <div className="rounded-sm border border-[var(--ink-200,#e5e5e5)] bg-[var(--paper-base,#faf9f7)] p-4 sticky top-6">
          {match && match.count > 0 ? (
            ...
            <>
              ...
              <p className="text-sm text-[var(--ink-600,#555)]">Pick what you are buying and this panel shows, live, how many verified vendors and managed providers match your project. Publishing is always your explicit choice.</p>
            </>
          )}
