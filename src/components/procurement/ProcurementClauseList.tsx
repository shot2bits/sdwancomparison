  ...
  if (clauses.length === 0) {
    return (
      <div className="border-t border-[#e3e1de] pb-4 pt-[18px]">
        ...
        <p className="mt-2 text-[13.5px] leading-[1.55] text-[#66635e]">
  ...
  return (
    <div className="border-t border-[#e3e1de] pb-4 pt-[18px]">
      <div className="mb-1 flex items-baseline gap-[11px]">
        ...
        <span className="min-w-0 flex-1 text-[12.5px] text-[#66635e]">every clause carries a stable id and traces to your own words or a named rule</span>
        <span className="flex-none text-[11px] text-[#66635e]" style={mono}>{clauses.length}</span>
      </div>
      {sections.map((section) => (
        <div key={section} className="pt-3">
          <div className="mb-1.5 text-[11.5px] font-medium text-[#66635e]">{SECTION_TITLES[section]}</div>
...
function ClauseRow({ clause, changed }: { clause: ProcurementClause; changed: boolean }) {
  return (
    <div className={`flex items-start gap-3.5 border-b border-dotted border-[#e3e1de] py-[10px] ${changed ? "ldoc-changed" : ""}`}>
      <span
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
          <span className="text-[10px] text-[#66635e]" style={mono}>{clause.id}</span>
          <span className="text-[14.5px] font-medium leading-[1.45] text-[#110f0d]" style={{ textWrap: "pretty" }}>
            {clause.statement}
          </span>
          <span
            ...
            style={{
              ...
              ...(clause.mandatory
                ? { background: "var(--nf-red-soft, #ffdfda)", color: "var(--nf-red, #8d1a1e)" }
                : { background: "var(--nf-neutral-tag-soft, #e7e4e0)", color: "var(--nf-neutral-tag, #47413a)" }),
            }}
          >
            {clause.mandatory ? "mandatory" : `scored · weight ${clause.weight}`}
          </span>
        </div>
        {clause.quote && <p className="m-0 mt-1 text-[12px] italic leading-[1.5] text-[#66635e]">&ldquo;{clause.quote}&rdquo;</p>}
        {clause.supplierResponse.length > 0 && (
          <ul className="m-0 mt-1.5 flex list-none flex-col gap-0.5 p-0 text-[12px] leading-[1.5] text-[#66635e]">
      ...
      <span className="flex-none pt-[2px] text-[10px] text-[#a7a4a0]" style={mono}>
