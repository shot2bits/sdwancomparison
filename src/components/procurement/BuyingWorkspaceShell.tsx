  ...
  return <div className="nf-buying-shell" data-view={view} data-collapsed={collapsed}>
    ...
    <div className="nf-buying-body">
      ...
      <div className="nf-buying-page">
        ...
        <div hidden={view !== 'project'} className="nf-buying-engine"><section className="nf-buying-start" aria-label="Ways to start your project"><nav aria-label="Start your buying journey"><button type="button" onClick={() => { navigate('project'); window.requestAnimationFrame(() => { const input = document.querySelector<HTMLTextAreaElement>('textarea[aria-label="Describe your requirements"]'); input?.focus(); input?.scrollIntoView({block:"center",behavior:"smooth"}); }); }}>Describe what you need</button><button type="button" onClick={() => { fireNetifyEvent('buying_route_selected', { intent: 'research' }); navigate('compare'); }}>Find suitable providers</button><a href="/sase/pricing/" onClick={() => fireNetifyEvent('buying_route_selected', { intent: 'pricing' })}>Request pricing</a><button type="button" onClick={() => { fireNetifyEvent('buying_route_selected', { intent: 'project' }); projectTool('import'); }}>Bring an RFP or RFI</button></nav></section>{children}</div>
