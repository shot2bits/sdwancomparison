...
export function publicationProjectFromReturn(value: string): string | null {
  ...
  if (!['/sase/home', '/sase/home/', '/sase/workspace', '/sase/workspace/', '/sase-sd-wan-rfp-builder', '/sase-sd-wan-rfp-builder/'].includes(url.pathname)) return null;
  const project = url.searchParams.get('project') || url.searchParams.get('id');
