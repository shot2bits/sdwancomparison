...
export type MarketplaceFunnelEvent = "project_started" | "requirements_updated" | "match_previewed" | "publication_prepared" | "publication_completed" | "publication_incomplete" | "supplier_interest" | "supplier_response";
...
export async function recordMarketplaceFunnelEvent(input: { event: MarketplaceFunnelEvent; project_id: string; source?: string; mode?: string; channel: "web" | "api" | "mcp" | "system"; detail?: Record<string, string | number | boolean | null> }) {
  ...
  try { await kvRaw(["LPUSH", "marketplace:funnel:events", JSON.stringify(record)]); await kvRaw(["LTRIM", "marketplace:funnel:events", "0", "9999"]); } catch { /* analytics never changes the buyer outcome */ }
  return record;
}
