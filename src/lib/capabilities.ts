...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    description: "Post an opportunity or a quick pricing request for underlay circuits or overlay SD-WAN/SSE/SASE. Pull-based: verified suppliers discover and respond.",
    access: "open", page: "/opportunities", api: "/api/opportunity (POST)", mcp: "list_opportunities", data: "/opportunities/board/data.json",
    status: "live", capabilityType: "compute", accessLevel: "public",
    requiresIdentity: false, requiresApproval: false, sendsExternally: false, executesExternally: false,
    invocableByExternalAgent: true, humanSupervision: "none",
    evidence: "/api/opportunity POST creates a board entry visible at /opportunities/board.",
    boundaries: "Publishes to the Netify public board (pull-based). Does not push to, email or contact any named supplier; pricing amounts stay private to the buyer.",
  },
  {
    id: "read_board",
  ...
  },
  {
    ...
    description: "Invite the best-fit graded vendors to a built RFP and move it to published. Push action: reaches named suppliers. Needs a buyer magic-link sign-in or the RFP manage_token.",
  ...
  },
  {
    ...
    description: "A verified supplier reads invitations and the board and submits comments, bids and structured pricing.",
    access: "identified", page: "/for-suppliers", mcp: "opportunity_inbox, opportunity_respond, get_rfp, respond_to_rfp",
    status: "live", capabilityType: "external-execution", accessLevel: "token-gated",
    requiresIdentity: true, requiresApproval: false, sendsExternally: true, executesExternally: true,
    invocableByExternalAgent: true, humanSupervision: "required",
    evidence: "opportunity_respond / respond_to_rfp MCP tools submit a supplier's own bid; domain-verified sign-in or per-supplier token required.",
    boundaries: "Acts only for the verified supplier's own organisation, on its own behalf. Cannot act for another supplier or for a buyer.",
  },
  ...
  },
  {
    ...
    description: "When a supplier submits a bid, the agent reviews it automatically without a buyer prompt: deterministic evidence checks (required-question coverage, hedging detection, compliance must-have coverage via the regulation engine) kept separate from an LLM quality judgement, a cross-check of the supplier's claim against Netify's independent vendor grade with overreach flagged, and drafted clarification questions.",
    ...
    boundaries: "Drafts clarifications only. Does not send them. Nothing reaches a supplier without explicit buyer approval.",
  },
  {
    ...
    boundaries: "Creates a buyer-only digest and pending proposals. Sends nothing (sends=0). Does not contact any supplier, account manager or BT. The /api/agent/run trigger is cron-secret or admin gated, not callable by an arbitrary agent.",
  },
];
...
export function capabilitiesDocument() {
  return {
    ...
    approval_model: {
      ...
      external_actions_require_human: "Any customer-facing, supplier-facing, BT-facing or buyer-impacting action requires a human: a sign-in or token to publish/bid, and explicit approval to send a drafted clarification or outreach.",
    ...
    flag_legend: {
      ...
      sendsExternally: "true if it pushes to a named customer, supplier or BT",
