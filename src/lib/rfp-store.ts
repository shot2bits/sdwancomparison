...
export type SignupAttribution = { ref: string; landing: string; page: string; country: string };

export async function createMagicToken(payload: { role: "supplier" | "buyer" | "netify"; email: string; vendor_slug: string | null; rfp_id?: string | null; attr?: SignupAttribution | null }): Promise<string> {
...
export async function consumeMagicToken(token: string): Promise<{ role: "supplier" | "buyer" | "netify"; email: string; vendor_slug: string | null; rfp_id: string | null; attr: SignupAttribution | null } | null> {
  const data = await getJson<{ role: "supplier" | "buyer" | "netify"; email: string; vendor_slug: string | null; rfp_id?: string | null; attr?: SignupAttribution | null; expires: number }>(`auth:magic:${token}`);
  if (!data || data.expires < Date.now()) return null;
  await kv(["DEL", `auth:magic:${token}`]);
  return { role: data.role, email: data.email, vendor_slug: data.vendor_slug, rfp_id: data.rfp_id ?? null, attr: data.attr ?? null };
}
