...
export function getShortlistDataset(): ShortlistVendor[] {
  return getAllVendors().map((v) => ({
    ...
    sectors: v.sectors,
    organisation_fit: v.organisation_fit,
    pricing_units: v.pricing_units,
    identity_providers: v.identity_providers,
    device_posture: v.device_posture,
    agent_platforms: v.agent_platforms,
    pop_count: v.pop_count,
    sla_availability_pct: v.sla_availability_pct,
