...
type Row = { environment: ActivityEnvironment; source: string; mode: string; channel: string; counts: Record<FunnelEvent, number> };
type Event = { environment: ActivityEnvironment; classification: string; at: number; event: FunnelEvent; project_id: string; source: string; mode: string; channel: string };
export function aggregateFunnel(raw: unknown[], now = Date.now()) {
 ...
 for (const item of raw) {
  try {
   ...
   events.push({ environment:activityEnvironment(e.environment ?? "unknown"), classification:e.classification === "test" || e.test === true ? "test" : "unverified", at:e.at, event:e.event, project_id:e.project_id, source:knownValue(e.source,FUNNEL_SOURCES), mode:knownValue(e.mode,FUNNEL_MODES), channel:knownValue(e.channel,FUNNEL_CHANNELS,"system") });
