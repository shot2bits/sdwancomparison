...
export async function GET(req: Request) {
  ...
  const lines = [
    ...
    `- Supplier sign-ins: ${supplierSessionsNew.length}`,
    ...
    `- Supplier access requests waiting in the queue: ${pending.length}`,
    "",
    "Running totals:",
    ...
    `- Supplier responses: ${responsesTotal}${delta(responsesTotal, snapshot?.responses)}`,
