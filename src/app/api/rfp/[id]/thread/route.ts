...
/**
 * POST creates a supplier question, or a buyer answer when { answer, thread_id }
 * is sent. Supplier questions are auto-categorised by keyword if not given.
 */
export async function POST(req: Request, ctx: Ctx) {
  ...
  let body: { vendor?: string; question?: string; category?: string; thread_id?: string; answer?: string };
  try {
    body = await req.json();
  } catch {
