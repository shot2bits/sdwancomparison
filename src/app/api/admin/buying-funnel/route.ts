...
export async function GET(req:Request) {
 ...
 for (let offset = 0; offset < ids.length; offset += 25) {
  await Promise.all(ids.slice(offset, offset + 25).map(async id => { if (activityClassification(await kvGetJson(`rfp:${id}`)) === "test") tests.add(id); }));
 }
 return Response.json(aggregateFunnel(parsed.map(e => e && tests.has(e.project_id) ? {...e, classification:"test"} : e)),{headers});}
